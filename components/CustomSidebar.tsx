...
import { Lock, Check, Menu, X, Clock } from "lucide-react";
...
export default function CustomSideBar() {
  ...
  const [completedDaysValue, setCompletedDaysValue] = useState<DayItem[]>([]);
  const [selectedDayValue, setSelectedDayValue] = useState<DayItem>();
  const [incompletedDaysValue, setInCompletedDaysValue] = useState<DayItem[]>(
    [],
  );
  ...
  const handleUnlockDay = (completeDayCount: number) => {
    ...
    setCompletedDaysValue([...completedDays]);
    setInCompletedDaysValue([...incompletedDays]);
    setSelectedDayValue(nextDay);
  ...
  const handleDayChange = (
    ...
    const allDayData = daysData.map((d) => {
      if (d.day === dayValue) {
        return {
          ...d,
          selected: true,
          completed: completed ? completed : d.completed,
        };
      }
      if (makeNextDayActive) {
        if (d.day === dayValue + 1) {
    ...
    const beforeDays = allDayData.slice(0, index);
    const afterDays = allDayData.slice(index + 1);
    setCompletedDaysValue([...beforeDays]);
    setInCompletedDaysValue([...afterDays]);
    setSelectedDayValue(chosenDay);
    dispatch(
      setSelectedDayData({
        ...chosenDay,
        completed: completed ? completed : chosenDay?.completed,
        ...
        >
          ...
          >
            {loading ? (
              ...
              completedDaysValue.map((item, index) => {
                return (
                  <div
                    key={item.day}
        ...
        >
          ...
          <div ref={scrollRef} className="h-full overflow-y-auto space-y-5 ">
            {loading ? (
              ...
              selectedDayValue && (
                <div
                  key={selectedDayValue.day}
                  // ref={(el) => (itemRefs.current[index] = el) as any}
                  onClick={() => handleDayChange(selectedDayValue.day)}
                  className={clsx(
                ...
                >
                  {!selectedDayValue.completed && (
                    <Clock color={theme === "dark" ? "white" : "black"} />
                  )}

                  <span
                    className={clsx(
                      "text-lg font-medium tracking-wide ml-3",
                      theme === "dark"
                        ? selectedDayValue.selected
                          ? "text-white"
                          : "text-gray-400"
                        : selectedDayValue.selected
                          ? "text-gray-900"
                  ...
                  {selectedDayValue.locked && !selectedDayValue.completed && (
                    <Lock
                      className={clsx(
                        "w-5 h-5",
                        theme === "dark" ? "text-gray-500" : "text-gray-600",
        ...
        >
          ...
          >
            {loading ? (
              ...
              incompletedDaysValue.map((item, index) => {
                return (
                  <div
                    key={item.day}
                    ref={(el) => (itemRefs.current[index] = el) as any}
